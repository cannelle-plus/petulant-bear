...
let updateProjection connection name eventId=
...
let isEventProcessed connection name eventId =
...
let withEvent connection name f escus (resolvedEvent:ResolvedEvent)  =
    ...
    if not <| isEventProcessed connection name meta.messageId then
        f meta jsonEvent 
