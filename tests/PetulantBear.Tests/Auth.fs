module PetulantBear.Tests.Auth
...
let auth_Tests =
  testList "auth tests" [
    testCase "baseline, no auth cookie" <| fun _ ->
      ...
      Assert.Null("should not
       have auth cookie", cookies.[Auth.SessionAuthCookie])
      
    testCase "can set cookie" <| fun _ ->
      let t = Auth.authenticated Session false >>= OK "ACK"
      let ctx = runWithConfig t
      let cookies = ctx |> reqCookies' HttpMethod.GET "/"  None
